...
策略内置**三层风险控制机制**，根据实时余额和美股开盘时段自动切换运行模式。每 10 次迭代检查一次余额，若余额跨越阈值则自动调整挂单参数。
...
2. **定期检查**：监控循环中每 10 次迭代检查一次
3. **降级优先级**：
   - **第1优先**：美股开盘时段（如启用）→ 强制 degraded_2
   - **第2优先**：余额检查 → 根据阈值判断模式
4. **自动切换**：模式变化时取消当前订单并重新挂单
5. **故障容错**：余额查询失败时保持当前模式，继续运行

### 美股开盘时段强制降级

启用 `MARKET_MAKER_FORCE_DEGRADED_ON_US_OPEN=true` 后：

- 美东时间周一-周五 09:30-16:00 自动进入 degraded_2 模式（80 bps）
- 自动处理冬夏令时切换，无需手动调整
- 优先级高于余额判断，开盘时段内不受余额影响
- 收盘后恢复正常余额判断逻辑
